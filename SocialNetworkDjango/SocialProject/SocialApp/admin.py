    ...
    def stats(request):
        ...
        if year:
            if quarter and month:
                result = result.filter(Q(created_date__quarter=quarter) | Q(month=month), created_date__year=year)
                stats_count_user = stats_count_user.filter(Q(date_joined__quarter=quarter) | Q(month=month),
